...
from config import MANHATTAN_NUM_UNBINNED, MANHATTAN_PEAK_MAX_COUNT, MANHATTAN_PEAK_PVAL_THRESHOLD, MANHATTAN_PEAK_SPRAWL_DIST, MANHATTAN_PEAK_VARIANT_COUNTING_PVAL_THRESHOLD
...
def extract_variants(phenocode : str, min_maf : float, max_maf : float, indel : bool) -> list:

    
    # load best of file
    # it's just a tsv of max 100 000 rows so I think the best option would be polars (instead of pandas) for this.
    df = pl.read_csv(current_app.config['BEST_OF_PHENO_DIR'] + f"/{phenocode}", 
                    separator = "\t",
                    dtypes = {'chrom' : str, 'pos' : int, 'ref': str, 'alt' : str, 'rsids' : str, 'nearest_genes' : str, 'pval': float, 'beta' : float, 'sebeta' : float, 'af' : float}
                    )
    
    # make sure it's MAF
    df = df.with_columns(
        pl.when(pl.col("af") > 0.5).then(1 - pl.col("af")).otherwise(pl.col("af")).alias("maf")
    )
...
class Binner:
    def __init__(self):
        ...
        assert MANHATTAN_PEAK_VARIANT_COUNTING_PVAL_THRESHOLD < MANHATTAN_PEAK_PVAL_THRESHOLD # counting must be stricter than peak-extending
    ...
    def process_variant(self, variant: Dict[str, Any]) -> None:
        ...
        if variant['pval'] < MANHATTAN_PEAK_PVAL_THRESHOLD: # part of a peak
            if self._peak_best_variant is None: # open a new peak
                ...
                self._num_significant_in_current_peak = 1 if variant['pval'] < MANHATTAN_PEAK_VARIANT_COUNTING_PVAL_THRESHOLD else 0
            elif self._peak_last_chrpos[0] == variant['chrom'] and self._peak_last_chrpos[1] + MANHATTAN_PEAK_SPRAWL_DIST > variant['pos']: # extend current peak
                if variant['pval'] < MANHATTAN_PEAK_VARIANT_COUNTING_PVAL_THRESHOLD: self._num_significant_in_current_peak += 1
                self._peak_last_chrpos = (variant['chrom'], variant['pos'])
