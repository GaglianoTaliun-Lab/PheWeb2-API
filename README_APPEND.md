...
When appending new GWAS data, you should ideally use the same set of variable–value pairs that were used for the first time ingest.
...
Your new manifest file should includes the new set of phenotypes to ingest.

You may follow the same steps described for the first time ingest!

> [!NOTE]
> The new manifest file can includes existing phenotypes as well but it is not required. Existing phenotypes won't be reprocessed. If the manifest file contains an existing phenotype with new stratifications, these stratifications will be ingested. We are referring to existing phenotypes as phenotypes already ingested. Previous pheno-list.json file used for the first time ingest is no longer needed.
