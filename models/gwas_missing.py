...
class SNPFetcher:
    ...
    def group_snps_by_region(self, snp_list):
        ...
        """
        snp_list = sorted(snp_list, key=lambda x: (x.split('-')[0], int(x.split('-')[1])))
        ...
        for snp in snp_list:
            ...
            pos = int(pos)
            if not current_group or (current_chrom == chrom and pos - current_start <= self.window_size):
    ...
    def fetch_snp_info_with_tbi(self, key, snp_list):
        ...
        for (chrom, start), snps in grouped_snps.items():
            ...
            try:
                ...
                print(f"Error fetching region {chrom}:{region_start}-{region_end} -> {e}")
