...
class _ivfr:
    ...
    def _parse_variant_row(self, variant_row:List[str]) -> Dict[str,Any]:
        ...
        for field in self._colidxs:
            ...
            try:
                ...
                raise Exception('ERROR: Failed to parse the value {!r} for field {!r} in file {!r}'.format(val, field, self._tabix_file.filename))
    ...
    def get_region(self, chrom:str, start:int, end:int) -> Iterator[Dict[str,Any]]:
        ...
        try:
            ...
            raise Exception('ERROR when fetching {}-{}-{} from {}'.format(chrom, start-1, end-1, self._tabix_file.filename)) from exc
