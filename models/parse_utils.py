...
class Field:
    ...
    def parse(self, value):
        ...
        if 'could_be_neglog10' in self._d and self._d['could_be_neglog10'] and current_app.config['PVAL_IS_NEGLOG10']:
            x = 10**-x
        ...
        if 'proportion_sigfigs' in self._d:
            if 0 <= x < 0.5:
                ...
                raise Exception('cannot use proportion_sigfigs on a number outside [0-1]')
...
if any(not field_name.islower() for field_name in fields):
    raise Exception("All field names must be lowercase, but these aren't: {}".format([fn for fn in fields if not fn.islower()]))
...
for field_name, field_dict in fields.items():
    for alias in field_dict['aliases']: # type: ignore
        ...
        if alias in default_field_aliases and default_field_aliases[alias] != field_name:
            raise Exception('The field_alias {!r} points to two fields: {!r} and {!r}'.format(alias, field_name, default_field_aliases[alias]))
