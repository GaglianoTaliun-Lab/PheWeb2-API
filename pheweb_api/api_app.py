...
from .conf import get_cors_origins, is_debug_mode, get_host, get_port, get_num_api_workers, get_api_url_prefix
load_dotenv()
import gunicorn.app.base
from .blueprints.cache import init_cache
import logging

class PrefixedApi(Api):
    def _register_apidoc(self, app):
        if self._doc is False:
            return
        if apidoc.apidoc.name in app.blueprints:
            return
        url_prefix = self.blueprint.url_prefix if getattr(self, "blueprint", None) else None
...
def create_app(enable_cache:bool=True) -> Flask:
    ...
    api_url_prefix = get_api_url_prefix()
    if api_url_prefix:
        if is_debug_mode(): print(f"DEBUG: api url prefix -> {api_url_prefix}")
        # app.register_blueprint(apidoc.apidoc, url_prefix = api_url_prefix)
        bp = Blueprint("api_v1", __name__, url_prefix = api_url_prefix)
        api_parent = bp
    else:
        api_parent = app
    ...
    api = PrefixedApi(
            ...
            doc="/docs",
            version="1.0", 
            title="PheWeb2 API Documentation", 
            description="Autogenerated documentation for PheWeb2 API.")
