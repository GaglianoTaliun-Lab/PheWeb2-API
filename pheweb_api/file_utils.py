...
_single_filepaths: Dict[str, Callable[[], str]] = {
    ...
    "phenomask": (lambda: get_generated_path("pheno-mask.json")),
    ...
    "interaction": (lambda: get_generated_path("interaction")),
...
_pheno_filepaths: Dict[str, Callable[[str], str]] = {
    ...
    "matrix-stratified": (
        lambda phenocode: get_generated_path(
            "matrix-stratified", "matrix{}.tsv.gz".format(phenocode)
        )
    ),
}
...
def get_matrices_filpaths() -> List[str]:
    """
    Returns all matrices filepaths with their stratification path.
    """
    ...
    if conf.has_stratifications():
        for stratification_path in stratification_paths:
            ...
            matrices.append(matrix_gz_stratified_filepath)
    else:
        matrices.append(get_filepath("matrix", must_exist=False))

    matrices = [m for m in matrices if os.path.exists(m)]
...
class MatrixReader:
    def __init__(self, matrix_filepath: Optional[str] = None):
        self._filepath = (
            get_generated_path("matrix.tsv.gz")
