...
def convert_sde(pheno: Dict[str, Any], ignore=None) -> None:
    ...
    phenocode_with_suffixes = get_phenocode_with_suffixes(pheno)
    suffixes = ".".join(phenocode_with_suffixes.split(".")[1:])

    sde_stratifications = conf.get_sde_map()[suffixes]

    strat1_pheno = ".".join(pheno["phenocode"], sde_stratifications[0])
    strat2_pheno = ".".join(pheno["phenocode"], sde_stratifications[1])

    parsed_strat1_filepath = get_pheno_filepath(
        "parsed", strat1_pheno, must_exist=False)
    parsed_strat2_filepath = get_pheno_filepath(
        "parsed", strat2_pheno, must_exist=False)
    ...
    with VariantFileReader(sites_filepath) as sites_reader, \
            ...
            VariantFileReader(parsed_strat1_filepath) as pheno_strat1_reader, \
            VariantFileReader(parsed_strat2_filepath) as pheno_strat2_reader, \
        ...
        strat1_variants = with_chrom_idx(iter(pheno_strat1_reader))
        strat2_variants = with_chrom_idx(iter(pheno_strat2_reader))
